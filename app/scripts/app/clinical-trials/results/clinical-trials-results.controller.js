	    ...
	    function ($scope, $state, $stateParams, $timeout, ClinicalTrialsService, TrialMatchService, $log, ToastService, trialSearch, trialMatches, uniqueFilter, ElasticSearchService, $filter, ENV) {
			...
			if (trialSearch) {
				...
				if (window.toPatientPage) {
					TrialMatchService.setProtocolNos(ctr.trials.map(function(trial){return trial._source.protocol_no}));
				}
		    ...
		    function getPaginationLimitedTrials(trials) {
				...
				if (ctr.sortOrder.indexOf('trial_rank') > -1) {
					...
					if (_.isEmpty(ctr.uniqueRanks)) {
						var ranks = ctr.trialMatches.map(function(item) {return {protocol_no: item.protocol_no, rank: item.sort_order}});
						ctr.uniqueRanks = _.uniq(ranks, function(item) {return item.protocol_no}).reduce(function(obj, value){
							obj[value.protocol_no] = value.rank;
							return obj;
						}, {});
					}
					uniqueRanks = ctr.uniqueRanks;
					trials.sort(function(a,b){return uniqueRanks[a._source.protocol_no] - uniqueRanks[b._source.protocol_no]});
				}
			...
			function sortMatches(matches) {
				matches = _.partition(matches, function(match) {return match.genomic_alteration.charAt(0) !== '!'});
				return _.flatten(matches.map(function(match) { return _.sortBy(match, 'match_type').reverse()}))
			}
			...
			ctr.onPaginationChange = function (page, limit) {
				if (ctr.hasTrialMatches) {
					...
					ClinicalTrialsService.paginate(page, limit).then(function(res){
						TrialMatchService.setProtocolNos(res.hits.hits.map(function(trial){return trial._source.protocol_no}))
					});
